...
from my_utils import test_single_volume, DiceLoss
...
def inference(args, model, testloader, multimask_output, device):
    ...
    logger = wandb.init(project='Test-AM-SAM', name=exp_name, resume='allow', anonymous='must', mode='online')
    logger.config.update(vars(args))

    # # ✅ 确保 mask 保存目录存在
    # mask_save_dir = os.path.join(args.output_dir, "predicted_masks")
    ...
    for i, sampled_batch in enumerate(tqdm(testloader)):
        ...
        ims['Test/Image'] = wandb.Image(image)
        ...
        ims['Test/Prediction'] = wandb.Image(output_masks)

        labs = labs.mul(255).to('cpu').numpy()
        ims['Test/GroundTruth'] = wandb.Image(labs)

        logger.log(ims)

        # # ✅ **额外新增**：保存 mask 到本地
        # mask_filename = sampled_batch['case_name'][0] + "_mask.png"
        # mask_save_path = os.path.join(mask_save_dir, mask_filename)
        #
        # # 转换数据格式并保存
        # output_mask_np = output_masks.squeeze().astype(np.uint8)
        # Image.fromarray(output_mask_np).save(mask_save_path)
        # print(f"✅ Mask saved at: {os.path.abspath(mask_save_path)}")
