...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        image = np.clip(image, 0, 1) # Ensure the image values are in the range [0, 1]
        ...
        # print("Inside RandomGenerator") #debug
        # print("Image min/max:", image.min(), image.max()) #debug
...
class Synapse_dataset(Dataset):
    def __init__(self, train_dir, num_data=0, transform=None, dataset=None):
        ...
        if self.num_data > 0:
            ...
            self.sample_list = [f for f in os.listdir(self.train_file) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]


        # print(f"Number of samples in dataset: {len(self.sample_list)}")#debug
    ...
    def __getitem__(self, idx):
        ...
        label_path = os.path.splitext(label_path)[0] + '_segmentation.png'
