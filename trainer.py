...
from my_utils import DiceLoss, getStat
...
def calc_loss(outputs, low_res_label_batch, ce_loss, dice_loss, dice_weight:float=0.8):
    ...
    # 🛠️ Debug: 打印 loss 相关信息
    # print(f"🔍 [calc_loss] loss_ce: {loss_ce.item():.6f}, loss_dice: {loss_dice.item():.6f}, total_loss: {loss.item():.6f}")
...
#
# @torch.no_grad()
# def validate(args, model, validloader, multimask_output, ce_loss, dice_loss):
#     score_dice = []
#     dice_loss_values = []
#     model.eval()
#
#     for i, sampled_batch in enumerate(validloader):
...
#         low_res_logits = outputs['low_res_logits']
#
#         preds = torch.argmax(torch.softmax(low_res_logits, dim=1), dim=1)
#         print(f"🔍 [validate] Batch {i}, Unique preds: {torch.unique(preds)}")
#         # Compute Dice Coefficient
#         dice_coef = dice_score(low_res_logits, low_res_label_batch)  # This is the Dice Coefficient
#         score_dice.append(dice_coef.cpu().numpy())
#
#         # Compute Dice Loss (which is different from Dice Coefficient)
#         _, _, dice_loss_value = calc_loss(outputs, low_res_label_batch, ce_loss, dice_loss, args.dice_param)
#         dice_loss_values.append(dice_loss_value.item())
#
#         print(f"🔍 [validate] Dice Coefficient: {dice_coef.item():.6f}, Dice Loss: {dice_loss_value.item():.6f}")
#
#     model.train()
#
#     avg_dice_coef = np.mean(score_dice)
#     avg_dice_loss = np.mean(dice_loss_values)
#
#     print(f"✅ [Validation] Avg Dice Coefficient: {avg_dice_coef:.6f}, Avg Dice Loss: {avg_dice_loss:.6f}")
#
#     return avg_dice_coef, avg_dice_loss
...
def trainer(args, model, snapshot_path, multimask_output, low_res):
    if args.dataset == 'kvasir':
        ...
        from datasets.dataset_ISIC import Synapse_dataset, RandomGenerator
    elif args.dataset == 'scannet':
        from datasets.dataset_scannet import Synapse_dataset, RandomGenerator
    elif args.dataset == 'body_2':
        from datasets.dataset_body_2 import Synapse_dataset, RandomGenerator
    else:
    ...
    print(f"Number of samples in dataset: {len(db_train)}")#debug
    ...
    #                          worker_init_fn=worker_init_fn, sampler=selector[num_train:])
    trainloader = DataLoader(db_train, batch_size=args.batch_size, num_workers=0, pin_memory=True,sampler=selector[:num_train]
                             )
    validloader = DataLoader(db_train, batch_size=args.batch_size, num_workers=0, pin_memory=True,sampler=selector[num_train:]
                             )
    ...
    #     print("🔍 Unique label values in batch:", unique_labels)  # Debug 信息
    #     break  # 只打印一个 batch

    # # 🛠️ Debug: 打印 trainloader 里的 label
    # for sampled_batch in trainloader:
    #     print(f"🔍 [trainloader] Unique label values: {torch.unique(sampled_batch    ['label'])}")
    #     break  # 只打印第一个 batch，避免输出太多
    #
    # # 🛠️ Debug: 打印 validloader 里的 label
    # for sampled_batch in validloader:
    #     print(f"🔍 [validloader] Unique label values: {torch.unique(sampled_batch['label'])}")
    #     break  # 只打印第一个 batch，避免输出太多
    ...
    for epoch_num in range(max_epoch):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print(f"Batch {i_batch + 1}") #debug
            # print("Image batch min/max:", image_batch.min(), image_batch.max()) # debug
            ...
            if iter_num % 10 == 0:
                ...
                output_new_masks = outputs['New_masks']
                output_original_masks = outputs['Original_masks']
                ...
                output_new_masks = torch.argmax(torch.softmax(output_new_masks, dim=1), dim=1, keepdim=True)[0, ...]
                output_new_masks = output_new_masks.mul(255).to('cpu').numpy()
                ims['train/Prediction_new_masks'] = wandb.Image(output_new_masks)

                output_original_masks = torch.argmax(torch.softmax(output_original_masks, dim=1), dim=1, keepdim=True)[
                    0, ...]
                output_original_masks = output_original_masks.mul(255).to('cpu').numpy()
                ims['train/Prediction_original_masks'] = wandb.Image(output_original_masks)
        ...
        wandb.log({'epoch': epoch_num + 1, 'valid_score': valid_score})
        logging.info('Epoch %d : valid score : %f' % (epoch_num + 1, valid_score))
        logger.log({'info/valid_score': valid_score})
        # validate the model at every epoch ending
        # valid_score, valid_loss = validate(args, model, validloader, multimask_output, ce_loss, dice_loss)
        #
        # wandb.log({
        #     'epoch': epoch_num + 1,
        #     'valid_score': valid_score,
        #     'valid_loss': valid_loss  # ✅ Log validation loss
        # })
        #
        # logging.info(f'Epoch {epoch_num + 1} : valid score : {valid_score:.6f}, valid loss : {valid_loss:.6f}')
        # logger.log({'info/valid_score': valid_score, 'info/valid_loss': valid_loss})  # ✅ Log to wandb
